...
function FilterReportsO() {
  ...
  if (search) {
    if (userData.worker && userData.worker.id) {
      url = `/api/bookings/date/worker/?page=${page}&search=${search}&from_date=${dates.fromDate}&to_date=${dates.toDate}&worker_id=${
    ...
    <div className="dashboard--content category reports">
      ...
      <div className="view-category-header filter">
        ...
        <i
          ...
          onClick={() =>
            navigate(
              `/owner/print-filterReport/?fromDate=${dates.fromDate}&toDate=${dates.toDate}&search=${search}&worker_id=${userData.worker && userData.worker.id}`
            )
          }
        ></i>
      </div>
