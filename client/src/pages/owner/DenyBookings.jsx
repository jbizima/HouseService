...
function DenyBookings() {
  ...
  if (search) {
    if (userData.worker && userData.worker.id) {
      url = `/api/booking/worker/?status=3&search=${search}&page=${page}&worker_id=${userData.worker.id}`;
    }
  } else {
    if (userData.worker && userData.worker.id) {
      url = `/api/booking/worker/?status=3&page=${page}&worker_id=${userData.worker.id}`;
    }
  }
  ...
  return (
    <div className="dashboard--content category">
      ...
      <BookTable bookings={data ? data : []} page="denied" />
      {isLoading && (
