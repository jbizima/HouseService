...
function DashboardOwnerHome() {
  ...
  if (userData.worker && userData.worker.id) {
    url1 = `/api/bookings/worker/?worker_id=${userData.worker.id}`;
  }
  const { data: bookings } = useFetchAll(url1);
  if (userData.worker && userData.worker.id) {
    url2 = `/api/bookings/worker/?status=2&worker_id=${userData.worker.id}`;
  }
  const { data: statusList } = useFetchAll(url2);

  const { data: dataBookings } = useFetchAll("/api/categories/bookings/");
  const dataSet = {};
  // console.log(data);
